      ...
      <div class="flex flex-col gap-3 py-1">
        ...
        <div class="flex flex-wrap gap-x-4 gap-y-1.5 text-xs">
          <label class="flex cursor-pointer items-center gap-1.5">
            <Checkbox
              :model-value="linkToolCalling ?? false"
              @update:model-value="linkToolCalling = toggleCheckbox(linkToolCalling, true)"
            />
            工具调用
          ...
          <label class="flex cursor-pointer items-center gap-1.5">
            <Checkbox
              :model-value="linkVision ?? false"
              @update:model-value="linkVision = toggleCheckbox(linkVision, true)"
            />
            视觉
            <span v-if="currentModel" class="ml-0.5 text-muted-foreground"
          ...
          <label class="flex cursor-pointer items-center gap-1.5">
            <Checkbox
              :model-value="linkThinking ?? false"
              @update:model-value="linkThinking = toggleCheckbox(linkThinking, true)"
            />
            思考
            <span v-if="currentModel" class="ml-0.5 text-muted-foreground"
          ...
          <label class="flex cursor-pointer items-center gap-1.5">
            <Checkbox
              :model-value="linkAdaptive ?? false"
              @update:model-value="linkAdaptive = toggleCheckbox(linkAdaptive, true)"
            />
            自适应思考
            <span v-if="currentModel" class="ml-0.5 text-muted-foreground"
