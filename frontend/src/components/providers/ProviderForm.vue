...
<template>
  <form ref="form" class="space-y-6" @submit.prevent="emit('submit')">
    <fieldset :disabled="saving" class="min-w-0 space-y-6">
      <div class="grid gap-4 sm:grid-cols-2">
        <div v-for="field in ['displayName', 'providerId'] as const" :key="field" class="space-y-2">
          ...
          />
          <p v-if="fieldErrors[field]" class="text-sm text-destructive">{{ fieldErrors[field] }}</p>
        </div>
      </div>
    ...
    <p v-if="error" role="alert" class="rounded border border-destructive p-3 text-destructive">
      {{ error }}
    </p>
    <div class="sticky bottom-0 flex flex-wrap justify-end gap-3 border-t bg-background py-4">
      <slot name="actions" /><Button
        ...
        @click="emit('cancel')"
        >取消</Button
      ><Button type="submit" :disabled="saving">{{ saving ? "保存中…" : submitLabel }}</Button>
    </div>
  </form>
</template>
