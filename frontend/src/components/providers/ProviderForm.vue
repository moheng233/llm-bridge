  ...
  <div class="grid grid-cols-1 gap-x-5 gap-y-3 max-h-[80vh] overflow-y-auto pr-1 lg:grid-cols-12">
    ...
    <div class="flex flex-col gap-3 lg:col-span-7">
      ...
      <div class="flex flex-col gap-2 pt-2 border-t border-border mt-1">
        ...
        <p v-if="apiKeys.length === 0" class="text-xs text-muted-foreground italic py-1">
          {{ isEdit ? "该提供者暂无 API Key，请添加" : "暂未配置 Key，可稍后在编辑界面补充" }}
        </p>
        <div v-else class="flex flex-col gap-2">
          <div v-for="(k, i) in apiKeys" :key="i" class="grid grid-cols-12 gap-2 items-center">
      ...
      <div class="flex flex-col gap-2 pt-2 border-t border-border mt-1">
        ...
        <p class="text-xs text-muted-foreground -mt-1">声明该提供者使用的上游额度查询协议。</p>
        <Select
        ...
        <div v-if="quotaAdapter" class="flex flex-col gap-2 pl-2 border-l-2 border-border">
    ...
    <div
      class="flex flex-col gap-2 pt-2 border-t border-border mt-1 lg:col-span-5 lg:pt-0 lg:border-t-0 lg:border-l lg:pl-5 lg:mt-0"
    >
      <div class="flex items-center justify-between">
        <Label>协议配置（可选）</Label>
        <Button
