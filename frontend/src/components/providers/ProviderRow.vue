...
<template>
  <div class="rounded-lg border border-border bg-card">
    ...
    >
      <ChevronDown v-if="expanded" class="h-4 w-4 text-muted-foreground shrink-0" />
    ...
    <div v-if="expanded" class="border-t border-border px-4 py-3 flex flex-col gap-4">
      <!-- API Keys 摘要（只读） -->
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <div
          ...
          class="rounded-md border border-border bg-muted/30 px-3 py-2 text-sm flex flex-col gap-1"
        >
          <div class="flex items-center gap-2">
          ...
          <pre
            ...
            class="text-xs font-mono text-muted-foreground whitespace-pre-wrap break-all m-0"
      ...
      <div class="flex flex-col gap-2">
        ...
        <p v-if="provider.protocols.length === 0" class="text-xs text-muted-foreground italic py-1">
        ...
        <div v-else class="flex flex-col gap-1.5">
          ...
          >
            <div class="flex items-center gap-2 min-w-0 flex-wrap">
              ...
              <Badge variant="outline" class="text-xs font-mono">{{ m.providerModelId }}</Badge>
              <Badge :variant="m.enabled ? 'default' : 'secondary'" class="text-xs">{{
                m.enabled ? "启用" : "禁用"
              }}</Badge>
              <span v-if="m.inputPricePer1m !== null" class="text-muted-foreground text-xs"
                >${{ m.inputPricePer1m }}/M in</span
              >
              <span v-if="m.outputPricePer1m !== null" class="text-muted-foreground text-xs"
